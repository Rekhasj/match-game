...
class MatchGame extends Component {
  state = {
    score: 0,
    time: 60,
    activeImageUrl: imagesList[0].thumbnailUrl,
  ...
  setTimeInterval = () => {
    this.timerId = setInterval(this.setCounter, 1000)
  }
  ...
  getRandomImage = () => {
    ...
    const activeImageId = imagesList[index].id
    const {score} = this.state
    const activeImageUrl = imagesList[index].thumbnailUrl

    this.setState({activeImageUrl, activeImageId, score: score + 1})
  }
  ...
  render() {
    const {score, time, activeImageUrl, activeTabId, isGameOver} = this.state
    const getFilteredCategoryList = this.getFilteredList()

    return (
      <div className="match-game-container">
        <Header score={score} time={time} />
        {isGameOver ? (
          <GameOver score={score} onClickReset={this.onClickReset} />
        ) : (
          <div className="game-container">
            <div className="display-image-card">
              <img alt="match" className="display-image" src={activeImageUrl} />
            </div>
            ...
            <ul className="thumbnail-container">
              {getFilteredCategoryList.map(eachImage => (
                <ThumbnailItem
